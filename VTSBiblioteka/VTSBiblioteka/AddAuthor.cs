...
namespace VTSBiblioteka
{
    public partial class AddAuthor : Form
    {
        ...
        {
            string query = "select Sifra, ImePrezime, Zemlja, Godina, Napomena from autori ORDER BY ImePrezime asc";
            SearchAuthor(query);
        }
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtSearch.Text))
            {
                ...
                {
                    string query = "select Sifra, ImePrezime, Zemlja, Godina, Napomena from autori where LOWER(Sifra) LIKE '%" + search_books.ToLower() + "%' ORDER BY ImePrezime asc";
                    SearchAuthor(query);
                }
                else if (comboSearch.Text == "po Imenu i Prezimenu")
                {
                    string query = "select Sifra, ImePrezime, Zemlja, Godina, Napomena from autori where LOWER(ImePrezime) LIKE '%" + search_books.ToLower() + "%' ORDER BY ImePrezime asc";
                    SearchAuthor(query);
                }
            }
            else
        ...
        {
            sql.Connection();
            bool result = sql.ReadAuthor(txtCode.Text, "autori");
            sql.Close();
            if (result)
            {
        ...
        {
            sql.Connection();
            bool authorRead = sql.AuthorRead(txtCode.Text, "autori");
            sql.Close();
            if (!authorRead)
            {
                sql.Connection();
                bool result = sql.InsertAuthor(txtCode.Text, txtName.Text, txtCountry.Text, txtYears.Text, txtNote.Text, "autori");
                sql.Close();
                if (result)
                {
                    Author("", "", "", "", "");
        ...
        private void btnChange_Click(object sender, EventArgs e)
        {
            ...
            {
                sql.Connection();
                bool result = sql.UpdateAuthor(txtCode.Text, txtName.Text, txtCountry.Text, txtYears.Text, txtNote.Text, "autori");
                sql.Close();
                if (result)
                {
                    Author("", "", "", "", "");
        ...
        {
            sql.Connection();
            bool result = sql.DeleteCode(txtCode.Text, "autori");
            sql.Close();
            if (result)
            {
                Author("", "", "", "", "");
