...
namespace VTSBiblioteka
{
    public partial class AddCategory : Form
    {
        ...
        {
            string query = "select Sifra, Naziv from kategorije ORDER BY Naziv asc";
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtSearch.Text))
            {
                ...
                {
                    string query = "select Sifra, Naziv from kategorije where LOWER(Sifra) LIKE '%" + search_books.ToLower() + "%' ORDER BY Naziv asc";
                    SearchCategory(query);
                }
                else if (comboSearch.Text == "po Nazivu")
                {
                    string query = "select Sifra, Naziv from kategorije where LOWER(Naziv) LIKE '%" + search_books.ToLower() + "%' ORDER BY Naziv asc";
                    SearchCategory(query);
                }
            }
            else
        ...
        {
            sql.Connection();
            bool result = sql.ReadCategory(txtCode.Text, "kategorije");
            sql.Close();
            if (result)
            {
        ...
        {
            sql.Connection();
            bool categoryRead = sql.CategoryRead(txtCode.Text, "kategorije");
            sql.Close();
            if (!categoryRead)
            {
                sql.Connection();
                bool result = sql.InsertCategory(txtCode.Text, txtName.Text, "kategorije");
                sql.Close();
                if (result)
                {
                    Category("", "");
        ...
        private void btnChange_Click(object sender, EventArgs e)
        {
            ...
            {
                sql.Connection();
                bool result = sql.UpdateCategory(txtCode.Text, txtName.Text, "kategorije");
                sql.Close();
                if (result)
                {
                    Category("", "");
        ...
        {
            sql.Connection();
            bool result = sql.DeleteCode(txtCode.Text, "kategorije");
            sql.Close();
            if (result)
            {
                Category("", "");
