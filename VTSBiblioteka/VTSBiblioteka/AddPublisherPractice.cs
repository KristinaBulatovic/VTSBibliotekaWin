...
{
    public partial class AddPublisherPractice : Form
    {
        MySQL sql;
        Config config;
        public AddPublisherPractice()
        {
            InitializeComponent();
            sql = new MySQL();
            config = new Config();
        ...
        private void AddPublisherPractice_Load(object sender, EventArgs e)
        {
            string query = "select Sifra, Naziv, Mesto, Adresa, Telefon, Web, Email from izdavaci_praksa ORDER BY Naziv asc";
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtSearch.Text))
            {
                ...
                {
                    string query = "select Sifra, Naziv, Mesto, Adresa, Telefon, Web, Email from izdavaci_praksa where LOWER(Sifra) LIKE '%" + search_books.ToLower() + "%' ORDER BY Naziv asc";
                    SearchPublisher(query);
                }
                else if (comboSearch.Text == "po Nazivu")
                {
                    string query = "select Sifra, Naziv, Mesto, Adresa, Telefon, Web, Email from izdavaci_praksa where LOWER(Naziv) LIKE '%" + search_books.ToLower() + "%' ORDER BY Naziv asc";
                    SearchPublisher(query);
                }
            }
            else
        ...
        private void AddPublisherPractice_FormClosing(object sender, FormClosingEventArgs e)
        {
            Publisher("", "", "", "", "", "", "");
        ...
        {
            sql.Connection();
            bool result = sql.ReadPublisher(txtCode.Text, "izdavaci_praksa");
            sql.Close();
            if (result)
            {
        ...
        {
            sql.Connection();
            bool publisherRead = sql.PublisherRead(txtCode.Text, "izdavaci_praksa");
            sql.Close();
            if (!publisherRead)
            {
                sql.Connection();
                bool result = sql.InsertPublisher(txtCode.Text, txtName.Text, txtLocation.Text, txtAddress.Text, txtPhone.Text, txtWeb.Text, txtEmail.Text, "izdavaci_praksa");
                sql.Close();
                if (result)
                {
                    Publisher("", "", "", "", "", "", "");
        ...
        private void btnChange_Click(object sender, EventArgs e)
        {
            ...
            {
                sql.Connection();
                bool result = sql.UpdatePublisher(txtCode.Text, txtName.Text, txtLocation.Text, txtAddress.Text, txtPhone.Text, txtWeb.Text, txtEmail.Text, "izdavaci_praksa");
                sql.Close();
                if (result)
                {
                    Publisher("", "", "", "", "", "", "");
        ...
        {
            sql.Connection();
            bool result = sql.DeleteCode(txtCode.Text, "izdavaci_praksa");
            sql.Close();
            if (result)
            {
                Publisher("", "", "", "", "", "", "");
